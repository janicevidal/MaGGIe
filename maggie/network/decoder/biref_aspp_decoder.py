...
class EfficientASPP(nn.Module):
    """
    Memory-efficient Atrous Spatial Pyramid Pooling.
    """

    def __init__(self, in_channels, out_channels, reduction_factor, dilation_rates, norm_cfg, act_cfg):
        ...
        self.reduce = ConvModule(in_channels, self.reduced_channels, kernel_size=1, bias=False, norm_cfg=norm_cfg, act_cfg=act_cfg)

        self.branches = nn.ModuleList([
            ConvModule(self.reduced_channels, self.reduced_channels, kernel_size=3, padding=rate, dilation=rate,
                groups=self.reduced_channels, bias=False, norm_cfg=norm_cfg, act_cfg=act_cfg)
            for rate in dilation_rates
        ])

        self.global_branch = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            ConvModule(self.reduced_channels, self.reduced_channels, kernel_size=1, bias=False, norm_cfg=norm_cfg, act_cfg=act_cfg))

        total_branches = len(dilation_rates) + 1
        self.fusion = ConvModule(
            self.reduced_channels * total_branches, self.reduced_channels, kernel_size=1, groups=self.reduced_channels, bias=False,
            norm_cfg=norm_cfg, act_cfg=act_cfg)

        self.expand = ConvModule(self.reduced_channels, out_channels, kernel_size=1, bias=False, norm_cfg=norm_cfg, act_cfg=act_cfg)
    ...
    def forward(self, x):
        ...
        x = self.fusion(torch.cat(branch_outputs, dim=1))
        return self.expand(x)


class BiRefASPPDecoder(nn.Module):
    def __init__(self, in_channels, ipt_channels, out_channels,
        ...
        super(BiRefASPPDecoder, self).__init__()
        ...
        self.easpp = EfficientASPP(in_channels=in_channels[0], out_channels=in_channels[0], reduction_factor=4, dilation_rates=[1, 6, 12, 18], norm_cfg=norm_cfg, act_cfg=act_cfg)
...
def biref_aspp_decoder(**kwargs):
    model = BiRefASPPDecoder(**kwargs)
    return model
